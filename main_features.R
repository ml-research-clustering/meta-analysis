...
setwd('C:/Users/Luiz_Henrique/Desktop/Luiz Henrique/Doutorado/Pesquisas de Tese/R/Clustering/Instance Space - ARI')
...
for(i in 1:5){ #length(gauss_ds)
  ...
  mds1[i, 1] <- intr_dim(gauss_ds[[i]])
  mds1[i, 2] <- contrast(gauss_ds[[i]])
  mds1[i, 3] <- hub_score(gauss_ds[[i]])
  mds1[i, 4] <- avg_nnd(gauss_ds[[i]])
  mds1[i, 5] <- power_cent(gauss_ds[[i]])
}
...
write.csv(mds6, "mds6.csv", row.names = FALSE)
